...
// BeShare's bundled self-signed key pair, loaded into a pooled ByteBuffer on each call.
// NOTE: do NOT cache these in a function-local static ByteBufferRef -- that Ref would
// outlive CompleteSetupSystem's teardown of muscle's ByteBuffer ObjectPool and trip an
// assertion at program exit.  Allocating per-call is cheap (small PEM strings) and the
// buffer is then owned by the MTT (which is destroyed well before shutdown).
// The private key ships in the binary, so TLS here is opportunistic encryption
// (defeats passive eavesdropping) rather than authentication.  See BeShareSSLKeys.h.
static muscle::ConstByteBufferRef GetTLSPrivateKeyBuffer()
{
   return muscle::GetByteBufferFromPool((uint32)strlen(kBeShareTLSPrivateKeyPEM), (const uint8 *)kBeShareTLSPrivateKeyPEM);
}
static muscle::ConstByteBufferRef GetTLSPublicKeyBuffer()
{
   return muscle::GetByteBufferFromPool((uint32)strlen(kBeShareTLSPublicKeyPEM), (const uint8 *)kBeShareTLSPublicKeyPEM);
}

// A ThreadWorkerSession that acts as the TLS *server* on its connection.  muscle's MTT only
// applies a private key to sockets it accepts via its own accept factory (the DoAccept path),
// NOT to sockets handed in via AddNewSession() -- which is how BeShare feeds the shared accept
// thread's already-accepted upload connections.  So for the direct-upload path we override
// CreateDataIO()/CreateGateway() to wrap the socket in server-mode SSL ourselves.
class SSLServerThreadWorkerSession : public muscle::ThreadWorkerSession
{
public:
   virtual muscle::DataIORef CreateDataIO(const muscle::ConstSocketRef & socket)
   {
      muscle::SSLSocketDataIO * ssl = new muscle::SSLSocketDataIO(socket, false /*non-blocking*/, true /*accept => TLS server*/);
      muscle::ConstByteBufferRef priv = GetTLSPrivateKeyBuffer();
      muscle::ConstByteBufferRef pub  = GetTLSPublicKeyBuffer();
      if ((priv())&&(pub()))
      {
         (void) ssl->SetPrivateKey(priv()->GetBuffer(), priv()->GetNumBytes());
         (void) ssl->SetPublicKeyCertificate(pub()->GetBuffer(), pub()->GetNumBytes());
      }
      return muscle::DataIORef(ssl);
   }
   virtual muscle::AbstractMessageIOGatewayRef CreateGateway()
   {
      // Wrap the normal gateway so OpenSSL's non-blocking state machine is driven correctly.
      muscle::AbstractMessageIOGatewayRef slave = muscle::ThreadWorkerSession::CreateGateway();
      return muscle::AbstractMessageIOGatewayRef(new muscle::SSLSocketAdapterGateway(slave));
   }
};
#endif

// Installs the bundled TLS keys onto (mtt) so its connection is encrypted.  The TCP-accepting
// side is the TLS server (needs BOTH private key and certificate); the TCP-connecting side is
// the TLS client (needs the certificate).  No-op unless the build has MUSCLE_ENABLE_SSL.
static void ApplyTLSToMTT(muscle::BMessageTransceiverThread * mtt, bool asServer)
{
#ifdef MUSCLE_ENABLE_SSL
   if (mtt == NULL) return;
   if (asServer)
   {
      (void) mtt->SetSSLPrivateKey(GetTLSPrivateKeyBuffer());
      (void) mtt->SetSSLPublicKeyCertificate(GetTLSPublicKeyBuffer());
   }
   else (void) mtt->SetSSLPublicKeyCertificate(GetTLSPublicKeyBuffer());
#else
   (void) mtt; (void) asServer;
#endif
}
...
public:
   ...
   void AddEntryRef(const entry_ref & ref, const char * path, off_t maxBytes = -1) {_entryRefs.AddTail(ref); _entryPaths.AddTail(path); _entryMaxBytes.AddTail(maxBytes);}
...
protected:
   ...
   Queue<off_t> _entryMaxBytes;   // per-file byte-range length for the request (-1 = whole file)
   MessageRef _msgRef;
   BMessenger _replyTo;
...
public:
   ...
   virtual void MessageReceived(BMessage *)
   {
      ...
      while(((_shutdownFlag == NULL)||((*_shutdownFlag) == false))&&((next = iter.GetNextKey()) != NULL))
      {
         ...
         entry_ref er;
         (void) ent.GetRef(&er);
         AddEntryRef(er, nextValue->_path(), nextValue->_maxBytes);
      }

      int numRefs = _entryRefs.GetNumItems();
      // Only emit the "maxbytes" byte-range fields when at least one file is bounded, so ordinary
      // whole-file requests stay byte-identical to stock BeShare.  When emitted, one is added per
      // file (aligned with "files"/"offsets") so the uploader can read them by index.
      bool anyBound = false;
      for (int i=0; i<numRefs; i++) if ((i < (int)_entryMaxBytes.GetNumItems())&&(_entryMaxBytes[i] >= 0)) { anyBound = true; break; }
      for (int i=0; i<numRefs; i++)
      {
         ...
         if (anyBound) msg.AddInt64("maxbytes", (i < (int)_entryMaxBytes.GetNumItems()) ? _entryMaxBytes[i] : (off_t)-1);
...
ShareFileTransfer(const BDirectory & fileDir, const char * localSessionID, uint64 remoteInstallID, uint64 partialHashSize, uint32 bandwidthLimit) :
   ...
   _currentFileStartOffset(0),
   _currentFileMaxBytes(-1),
   _saveLastFileSize(-1),
   ...
   _isFinished(false),
   _triedConnectBackFallback(false),
   _currentFileIndex(0),
   _origFileSetSize(0),
   _animShift(0.0f),
...
void 
ShareFileTransfer::
AddRequestedFileName(const char * fileName, off_t optStartByte, const char * path, BPoint *point, off_t maxBytes)
{
   String s(fileName);
   OffsetAndPath oap(optStartByte, path, maxBytes);
...
InitConnectSession(const char * hostName, uint16 port, uint32 remoteIP, const char * remoteSessionID)
{
   ...
   if (_useTLS) ApplyTLSToMTT(_mtt, false /*we connect => TLS client*/);
   return B_NO_ERROR;
}
...
InitAcceptSession(const char * remoteSessionID)
{
   ...
   _mtt = new BMessageTransceiverThread(toMe);
   if (_useTLS) ApplyTLSToMTT(_mtt, true /*we accept => TLS server*/);
   ...
   {
      // MUSCLE 6.11: the bound port is returned via the optRetPort out-param
      // (ReflectSessionFactory::GetPort() no longer exists).
      uint16 boundPort = 0;
      if ((ret = _mtt->PutAcceptFactory(((i<DEFAULT_LISTEN_PORT+LISTEN_PORT_RANGE)?i:0), factoryRef, invalidIP, &boundPort)) == B_NO_ERROR)
      {
         _acceptingOn = boundPort;
...
InitSocketUploadSession(const ConstSocketRef & socket, uint32 remoteIP, bool sendQueuedNotify)
{
   ...
   _mtt = new BMessageTransceiverThread(toMe);
   // The peer connected to us => we act as the TLS server, via a session that wraps the
   // already-accepted socket in server-mode SSL (see SSLServerThreadWorkerSession).
   status_t ret = (_mtt->AddNewSession(socket, GetTransferSessionRef(_useTLS)) == B_NO_ERROR) ? _mtt->StartInternalThread() : B_ERROR;
   if ((ret == B_NO_ERROR)&&(sendQueuedNotify)) SendNotifyQueuedMessage();
   return ret;
}

ThreadWorkerSessionRef
ShareFileTransfer :: GetTransferSessionRef(bool sslServer)
{
   ThreadWorkerSessionRef workerRef;
#ifdef MUSCLE_ENABLE_SSL
   // For the direct-upload (accept) path we must supply a session that does server-mode SSL
   // itself, because the MTT's built-in SSL only covers its own accept-factory sockets.
   if (sslServer) workerRef.SetRef(new SSLServerThreadWorkerSession);
#else
   (void) sslServer;
#endif
   if (_bandwidthLimit > 0)
   {
      if (workerRef() == NULL) workerRef.SetRef(new ThreadWorkerSession);
      ...
      case TRANSFER_COMMAND_MD5_RECV_READ_DONE:
      {
         ...
         if ((msg->FindPointer("from", (void **) &looper) == B_NO_ERROR)&&(_md5Loopers.Remove(looper) == B_NO_ERROR))
         {
            ...
            for (int i=0; (pmsg->FindString("files", i, &nextFile) == B_NO_ERROR); i++)
            {
               ...
               // Optional byte-range length (resume/swarming): if present, serve at most this
               // many bytes from (offset) instead of to end-of-file.  Absent => whole rest of file.
               int64 maxBytes = -1;
               (void) pmsg->FindInt64("maxbytes", i, &maxBytes);
               AddRequestedFileName(nextFile, offset, nextPath(), NULL, (off_t)maxBytes);
            }
               ...
               switch(code)
               {
                  ...
                  case MTT_EVENT_SESSION_DISCONNECTED:
                  {
                     ...
                     else
                     {
                        ...
                        {
                           ((ShareWindow*)Looper())->DoBeep(SYSTEM_SOUND_DOWNLOAD_FINISHED);
                           String detail = (_currentFileName.Length() > 0) ? _currentFileName : String("File transfer");
                           if (_remoteUserName.Length() > 0) { detail += " ("; detail += _remoteUserName; detail += ")"; }
                           ShowNotification("HiShare Downloads", "Download complete", detail.Cstr());
                        }
                     }

                     // Our outgoing connection to a supposedly non-firewalled peer never came
                     // up: his advertised address may be wrong (stale mapping, CGNAT, broken
                     // port forwarding).  Ask the window to retry this download once in
                     // accept/connect-back mode; deferred via message because our caller
                     // still needs to tear down the old _mtt below.  (User-initiated aborts
                     // go through AbortSession() directly and never reach this event.)
                     const bool retryViaConnectBack = ((_errorOccurred)&&(_uploadSession == false)&&
                                                       (_wasConnected == false)&&(_isAcceptSession == false)&&
                                                       (_triedConnectBackFallback == false));
                     if (retryViaConnectBack)
                     {
                        _triedConnectBackFallback = true;
                        BMessage retryMsg(ShareWindow::SHAREWINDOW_COMMAND_RETRY_VIA_CONNECT_BACK);
                        retryMsg.AddPointer("who", this);
                        Looper()->PostMessage(&retryMsg);
                     }

                     if ((_autoRestart == false)&&(retryViaConnectBack == false)) ((ShareWindow*)Looper())->FileTransferDisconnected(this);
                     if (_mtt)
                     {
                        _mtt->ShutdownInternalThread();
...
DoUploadAux()
{
   ...
   if (_mtt)
   {
      ...
      {
         // Byte-range bound (resume/swarming): once we've sent the requested number of bytes
         // from the start offset, treat it as end-of-file and move on to the next file.
         if ((_currentFileMaxBytes >= 0LL)&&((_currentFileBytesDone - _currentFileStartOffset) >= _currentFileMaxBytes))
         {
            _currentFileBytesDone = _currentFileSize = -1;
            return true;  // advance to the next file in the set
         }
                      ...
                      (msg()->FindDataPointer("data", B_RAW_TYPE, (void **) &scratchBuffer, NULL) == B_NO_ERROR))
         {
            ...
            // Don't read past the requested byte-range end.
            size_t toRead = bufferSize;
            if (_currentFileMaxBytes >= 0LL)
            {
               off_t remaining = _currentFileMaxBytes - (_currentFileBytesDone - _currentFileStartOffset);
               if ((off_t)toRead > remaining) toRead = (size_t)remaining;
            }
            ssize_t numBytes = _currentFile.Read(scratchBuffer, toRead);
            if (numBytes > 0) 
            {
      ...
      else 
      {
         ...
         if (_fileSetIter.GetNextKey(_currentFileName) == B_NO_ERROR)
         {
            ...
            _currentFileStartOffset = value->_offset;   // remember it so we can bound a byte-range request
            _currentFileMaxBytes    = value->_maxBytes; // -1 = send to EOF; >=0 = send at most this many bytes
            ...
            if (_currentFile.SetTo(&_currentFileEntry, B_READ_ONLY) == B_NO_ERROR)
            {
                   ...
                   (_currentFile.Seek(_currentFileBytesDone, SEEK_SET) == _currentFileBytesDone))
               {
                  ...
                  if (_currentFileMaxBytes >= 0LL) header()->AddInt64("beshare:SendLength", _currentFileMaxBytes);   // byte-range mode: only this many bytes from the offset are coming
...
DrawItem(BView * lv, BRect itemRect, bool /*complete*/)
{
   ...
   if (doubleBufferBitmap->Lock())
   {
            ...
            else
            {
               ...
               if (_transferStamps.GetNumItems() > 2)
               {
                  ...
                  if (elapsedTime > 0)
                  {
                     ...
                     // Add performance info, e.g. (45%, 43KB/sec, 5:05)
                     text += " (";

                     // Numeric percentage complete, alongside the progress bar.
                     if ((_currentFileSize > 0)&&(percent >= 0.0f))
                     {
                        char pbuf[12];
                        sprintf(pbuf, "%d%%, ", (int)(percent*100.0f + 0.5f));
                        text += pbuf;
                     }
      ...
      // Themed text colour: the row backgrounds are subtle tints of the theme
      // background, so the palette's text colour contrasts in light & dark mode.
      doubleBufferView->SetHighColor(win->GetColor(COLOR_TEXT));
      for (int i=0; i<2; i++)
      {
      ...
      doubleBufferView->SetHighColor(selected ? bevelLight : bevelDark);
