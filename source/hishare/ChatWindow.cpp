...
// The historical (light-theme) colours.  These are the base hues; the actual
// defaults used at runtime are derived from these plus the current system theme
// by ApplySystemColorsToDefaults(), so BeShare follows Haiku's light/dark mode.
static const rgb_color kBaseColors[NUM_COLORS] = {
...
// Runtime defaults (filled in by ApplySystemColorsToDefaults()).
static rgb_color _defaultColors[NUM_COLORS];

static inline float ColorLuminance(const rgb_color & c) { return 0.299f*c.red + 0.587f*c.green + 0.114f*c.blue; }
static inline uint8 ClampByte(int v) { return (uint8)((v < 0) ? 0 : (v > 255 ? 255 : v)); }

// Blend (a) toward (b) by fraction t (0..1).
static rgb_color BlendColors(const rgb_color & a, const rgb_color & b, float t)
{
   rgb_color r;
   r.red   = ClampByte((int)(a.red   + (b.red   - a.red)   * t));
   r.green = ClampByte((int)(a.green + (b.green - a.green) * t));
   r.blue  = ClampByte((int)(a.blue  + (b.blue  - a.blue)  * t));
   r.alpha = 255;
   return r;
}

// Nudge a foreground colour until it has enough luminance contrast against (bg),
// so saturated hues stay legible on both light and dark backgrounds.
static rgb_color EnsureContrast(rgb_color fg, const rgb_color & bg)
{
   float lb = ColorLuminance(bg);
   int guard = 0;
   while ((fabs(ColorLuminance(fg) - lb) < 96.0f) && (guard++ < 24))
   {
      int d = (lb < 128.0f) ? +12 : -12;   // lighten on dark bg, darken on light bg
      fg.red = ClampByte(fg.red + d); fg.green = ClampByte(fg.green + d); fg.blue = ClampByte(fg.blue + d);
   }
   return fg;
}

// Recompute _defaultColors from kBaseColors + the current system theme.  Called
// at construction and whenever the system colours change (B_COLORS_UPDATED).
static void ApplySystemColorsToDefaults()
{
   for (uint32 i = 0; i < NUM_COLORS; i++) _defaultColors[i] = kBaseColors[i];

   const rgb_color bg = ui_color(B_DOCUMENT_BACKGROUND_COLOR);
   const rgb_color tx = ui_color(B_DOCUMENT_TEXT_COLOR);
   const bool dark = (ColorLuminance(bg) < 128.0f);

   _defaultColors[COLOR_BG]      = bg;
   _defaultColors[COLOR_TEXT]    = tx;
   _defaultColors[COLOR_REMOTE]  = tx;   // remote chat is the main body text
   _defaultColors[COLOR_BORDERS] = ui_color(B_PANEL_BACKGROUND_COLOR);
   _defaultColors[COLOR_SCROLLBG] = tint_color(bg, dark ? 0.85f : 1.07f);

   // Keep the saturated text hues legible against the theme background.
   const uint32 hues[] = { COLOR_SYSTEM, COLOR_WARNING, COLOR_ERROR, COLOR_ACTION,
                           COLOR_PRIVATE, COLOR_NAMESAID, COLOR_PING, COLOR_LOCAL,
                           COLOR_URL, COLOR_WATCH };
   for (uint32 k = 0; k < sizeof(hues)/sizeof(hues[0]); k++)
      _defaultColors[hues[k]] = EnsureContrast(kBaseColors[hues[k]], bg);

   // The transfer-row background tints are pastels; blend them onto the theme
   // background so they read as a subtle tint in either light or dark mode.
   const uint32 rowBgs[] = { COLOR_DOWNLOAD, COLOR_UPLOAD, COLOR_PAUSEDUPLOAD, COLOR_SELECTION };
   for (uint32 k = 0; k < sizeof(rowBgs)/sizeof(rowBgs[0]); k++)
      _defaultColors[rowBgs[k]] = BlendColors(bg, kBaseColors[rowBgs[k]], dark ? 0.28f : 0.85f);
}
...
{
   // Derive the default colours from the current system theme so BeShare
   // follows Haiku's light/dark mode out of the box.
   ApplySystemColorsToDefaults();
...
void ChatWindow :: MessageReceived(BMessage * msg)
{
   switch(msg->what)
   {
      ...
      case B_COLORS_UPDATED:   // system light/dark theme changed
         SystemColorsChanged();
      break;
      ...
      case CHATWINDOW_COMMAND_COLOR_CHANGED:
      {
         ...
         {
            rgb_color temp;
            if (RestoreColorFromMessage("rgb", temp, *msg, i) == B_NO_ERROR)
            {
               // Picking a genuinely new color implies the user wants custom
               // colors on; the picker also echoes back the current color when
               // opened, which must not flip the setting.
               if (ColorsDiffer(temp, GetColor(color))) SetCustomColorsEnabled(true);
               SetColor(color, temp);
            }
         }
         UpdateColors();
...
ChatTextReceivedBeep(bool isPersonal, bool mentionsName)
{
   if ((isPersonal)||(mentionsName))
   {
      ...
      // Only pop a desktop notification when this window isn't the active one,
      // so we don't distract the user while they're already reading the chat.
      if (IsActive() == false)
         ShowNotification("HiShare Chat",
                          isPersonal ? "Private message" : "You were mentioned",
                          isPersonal ? "You received a private message in HiShare."
                                     : "Your name was mentioned in HiShare chat.");
   }
}
...
ChatWindow :: GetAppSubdir(const char * subDirName, BDirectory & subDir, bool createIfNecessary) const
{
   ...
   // BESHARE_HOME overrides the base directory (portable installs, multi-instance testing).

   const char * baseDirName = getenv("BESHARE_HOME");
   if (baseDirName == NULL) baseDirName = "/boot/home/HiShare";
   (void) create_directory(baseDirName, 0755);  // assume we always want this to exist
   BDirectory appDir(baseDirName);
...
SystemColorsChanged()
{
   // The system light/dark theme changed: recompute the derived defaults and
   // re-sync into the live palette only the structural colours the user never
   // customised (still equal to the previous defaults) — a colour the user
   // picked explicitly must survive theme changes, or it would also get
   // persisted wrong on the next settings save.
   rgb_color oldDefaults[NUM_COLORS];
   for (uint32 i=0; i<NUM_COLORS; i++) oldDefaults[i] = _defaultColors[i];
   ApplySystemColorsToDefaults();
   const uint32 structural[] = { COLOR_BG, COLOR_TEXT, COLOR_REMOTE, COLOR_SCROLLBG, COLOR_BORDERS };
   for (uint32 k = 0; k < sizeof(structural)/sizeof(structural[0]); k++)
      if (ColorsDiffer(_colors[structural[k]], oldDefaults[structural[k]]) == false)
         _colors[structural[k]] = _defaultColors[structural[k]];
   UpdateColors();
}
