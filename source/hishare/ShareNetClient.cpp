                     ...
                     case USER_NAME_DEPTH: 
                     {
                        ...
                        if (strncmp(nodeName, "name", 4) == 0)
                        {
                           ...
                           if (pmsg->FindString("name", &name) == B_NO_ERROR)
                           {
                              ...
                              // See if this client advertises that it requires/accepts TLS-encrypted transfers
                              bool ssl = false;
                              (void) pmsg->FindBool("supports_ssl", &ssl);

                              // See if this client honors "maxbytes" byte-range requests (resume/swarming)
                              bool ranges = false;
...
SetLocalUserName(const char * name)
{
   ...
   if (_mtt)
   {
      ...
      if (nameMessage())
      {
         ...
         // Identify ourselves!
         nameMessage()->AddString("version_name", "HiShare");
         ...
         nameMessage()->AddBool("supports_ranges", true);  // this build honors "maxbytes" byte-range requests (resume/swarming)
         if (_requireTLS) nameMessage()->AddBool("supports_ssl", true);  // we require/accept TLS-encrypted transfers
...
void
ShareNetClient ::
SetRequireTLS(bool requireTLS)
{
   if (requireTLS != _requireTLS)
   {
      _requireTLS = requireTLS;
      // Re-publish our name node so peers see the updated "supports_ssl" capability.
      if ((_mtt)&&(_localUserName.Length() > 0)) SetLocalUserName(_localUserName());
   }
}
...
void
ShareNetClient ::
SendConnectBackRequestMessage(const char * targetSessionID, uint16 port, bool useSSL)
{
   if (_mtt)
   {
      ...
      if (reqMsg())
      {
         ...
         if (useSSL) reqMsg()->AddBool("use_ssl", true);  // ask the peer to connect back with TLS
